...
export function NotionSetupGuide() {
  ...
  const [shoppingDatabaseId, setShoppingDatabaseId] = useState("");
  const [purchaseRecordsDatabaseId, setPurchaseRecordsDatabaseId] = useState("");
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/notion/setup", {
        ...
        body: JSON.stringify({ token, parentPage, shoppingDatabaseId, purchaseRecordsDatabaseId })
      });
      ...
      setSuccessMessage(payload.alreadyConfigured ? "Notion 配置已验证并写入本地配置。" : "Notion 数据库已补全并写入本地配置。");
      setToken("");
      setShoppingDatabaseId("");
      setPurchaseRecordsDatabaseId("");
      ...
      <div className="panel overflow-hidden">
        <div className="border-b border-slate-100 px-5 py-4">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-600">
            已检测到当前项目还没有完整的 Notion 数据库配置。如果已有数据库，可以填入已有数据库 ID；留空则只在父页面下创建缺失的数据库。
          </p>
        </div>
        ...
        <div className="grid gap-4 p-5">
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 sm:col-span-2">
              在 Notion 创建 Internal Integration，复制 token；再新建或选择一个父页面，把该页面以及已有数据库分享给这个 Integration。
            </div>
          </div>
          ...
          <form className="grid gap-3" onSubmit={handleSubmit}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="flex flex-col gap-1 text-sm font-medium text-slate-600">
                ...
                <input
                  className="field"
                  value={shoppingDatabaseId}
                  onChange={(event) => setShoppingDatabaseId(event.target.value)}
              ...
              <label className="flex flex-col gap-1 text-sm font-medium text-slate-600">
                ...
                <input
                  className="field"
                  value={purchaseRecordsDatabaseId}
                  onChange={(event) => setPurchaseRecordsDatabaseId(event.target.value)}
                  placeholder="Notion 数据库链接或 ID"
                />
              </label>
            </div>
