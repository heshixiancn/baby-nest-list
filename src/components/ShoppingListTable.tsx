...
export function ShoppingListTable({
  ...
  const baseGroups = useMemo(
    () => (groupOptions.length > 0 ? groupOptions : [...ITEM_GROUPS]),
    [groupOptions]
  );
  ...
  const [editForm, setEditForm] = useState<EditItemFormState>({
    ...emptyEditForm,
    group: initialGroup
  });
  const [editingItem, setEditingItem] = useState<ShoppingItem | null>(null);
  const [deleteTargetItem, setDeleteTargetItem] = useState<ShoppingItem | null>(
    null
  );
  const [pendingId, setPendingId] = useState<string | null>(null);
  const [quantityPendingId, setQuantityPendingId] = useState<string | null>(
    null
  );
  ...
  const [linkDialogItem, setLinkDialogItem] = useState<ShoppingItem | null>(
    null
  );
  ...
  const visibleGroups = useMemo(() => {
    const nextGroups = groups.filter(
      (groupName) => !hiddenGroups.includes(groupName)
    );
  ...
  const groupCounts = useMemo(() => {
    return visibleGroups.map((groupName) => {
      const itemsInGroup = localItems.filter(
  ...
  const printItems = useMemo(
    () => filteredItems.filter((item) => item.status !== "已放弃"),
    [filteredItems]
  );
  ...
  const groupStatistics = visibleGroups
    .map((groupName) => {
      const itemsInGroup = statisticsItems.filter(
        (item) => item.group === groupName
      );
      return {
        group: groupName,
        count: itemsInGroup.length,
        total: itemsInGroup.reduce(
          (sum, item) => sum + item.unitPrice * item.quantity,
          0
        )
      };
  ...
  const statusStatistics = SHOPPING_STATUSES.map((statusName) => {
    const itemsInStatus = statisticsItems.filter(
      (item) => item.status === statusName
    );
    return {
      status: statusName,
      count: itemsInStatus.length,
      total: itemsInStatus.reduce(
        (sum, item) => sum + item.unitPrice * item.quantity,
        0
      )
    };
  ...
  const paymentMethodStatistics = PAYMENT_METHODS.map((methodName, index) => {
    const itemsInMethod = statisticsItems.filter(
      (item) => item.paymentMethod === methodName
    );
    return {
      method: methodName,
      count: itemsInMethod.length,
      total: itemsInMethod.reduce(
        (sum, item) => sum + item.unitPrice * item.quantity,
        0
      ),
  ...
  useEffect(() => {
    setLocalGroups((current) =>
      Array.from(new Set([...baseGroups, ...current]))
    );
  }, [baseGroups]);
  ...
  async function handleCreateGroup(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (groups.includes(nextGroupName)) {
      ...
      setHiddenGroups((current) =>
        current.filter((groupName) => groupName !== nextGroupName)
      );
    ...
    try {
      ...
      const nextGroups = payload.groups?.length
        ? payload.groups
        : [...groups, createdGroup];
      triggerHapticFeedback("success");
      setLocalGroups(Array.from(new Set(nextGroups)));
      setHiddenGroups((current) =>
        current.filter((groupName) => groupName !== createdGroup)
      );
  ...
  async function handleMoveGroup(groupName: string, direction: "up" | "down") {
    ...
    try {
      ...
      triggerHapticFeedback("error");
      setError(
        moveError instanceof Error ? moveError.message : "保存分组顺序失败。"
      );
  ...
  async function handleQuantityChange(
    ...
    if (item.quantity === normalizedQuantity || quantityPendingId === item.id)
      return;
  ...
  async function handleEditItem(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      triggerHapticFeedback("error");
      setError(
        editError instanceof Error ? editError.message : "保存物品失败。"
      );
    } finally {
      setSavingEdit(false);
    }
  ...
  async function handleDeleteItem() {
    ...
    try {
      const response = await fetch(
        `/api/shopping-list/${deleteTargetItem.id}`,
        {
          method: "DELETE"
        }
      );
      ...
      setLocalItems(previousItems);
      setError(
        deleteError instanceof Error ? deleteError.message : "删除物品失败。"
      );
    } finally {
      setDeletingId(null);
    }
  ...
  function renderStatusMenu() {
    const item = openStatusMenuId
      ? localItems.find((currentItem) => currentItem.id === openStatusMenuId)
      : undefined;
      ...
      <div className="panel overflow-hidden">
        <div className="border-b border-slate-100 p-3 sm:p-4">
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:justify-end">
            ...
            <button
              className="btn btn-primary h-10 w-full px-2 sm:h-9 sm:w-auto sm:px-3"
              type="button"
        ...
        <div className="border-b border-slate-100 px-3 py-3 sm:px-4 sm:py-0">
          <div
            className="flex gap-2 overflow-x-auto pb-1 sm:gap-5 sm:pb-0"
            role="tablist"
            aria-label="采购清单分组"
          >
                ...
                >
                  ...
                  <article
                    className={cn(
                      ...
                      openActionsItemId === item.id
                        ? "-translate-x-[112px] border-amber-200"
                        : "translate-x-0"
                    )}
                    style={{ touchAction: "pan-y" }}
                    onPointerDown={(event) =>
                      handleCardPointerDown(item, event)
                    }
                  ...
                  >
                    ...
                    <div className="mt-4 grid grid-cols-[minmax(0,1fr)_minmax(7.25rem,0.8fr)] items-center gap-3">
                      ...
                      <div className="min-w-0">{renderStatusControl(item)}</div>
                    </div>
                  </article>
                </div>
              ))}
              ...
              <table className="min-w-[760px] w-full divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-200 text-sm">
                ...
                <tbody className="divide-y divide-slate-100 bg-white">
                  {filteredItems.map((item) => (
                    <tr
                      key={item.id}
                      className="align-top transition-colors duration-150 hover:bg-slate-50/70"
                    >
          ...
          <div className="soft-dialog panel w-full max-w-sm rounded-b-none sm:rounded-lg">
            <div className="border-b border-slate-100 px-5 py-4">
              <h2 className="text-base font-semibold text-slate-900">
                {linkDialogItem.name || "商品链接"}
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                {linkDialogItem.brandModel || "未填写品牌型号"}
              </p>
            </div>
            ...
            >
              ...
              <div className="grid gap-2 sm:col-span-2 sm:flex sm:flex-wrap lg:col-span-4">
                <button
                  ...
                  type="submit"
                  disabled={savingEdit || isPending}
                >
                  {savingEdit ? "正在保存..." : "保存修改"}
                </button>
                <button
                  className="btn h-10 w-full sm:w-auto"
                  type="button"
                  onClick={closeEditDialog}
                  disabled={savingEdit}
                >
        ...
        <div className="soft-backdrop fixed inset-0 z-50 flex items-end justify-center bg-slate-950/40 px-0 py-0 sm:items-center sm:px-4">
          <div className="soft-dialog panel w-full max-w-sm rounded-b-none sm:rounded-lg">
            ...
            <div className="grid gap-2 p-5">
              ...
              >
                {deletingId === deleteTargetItem.id
                  ? "正在删除..."
                  : "确认删除"}
              </button>
              <button
              ...
              <section className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-lg border border-amber-100 bg-amber-50 p-4">
                  ...
                  <p className="mt-2 text-2xl font-semibold text-amber-800">
                    {formatCurrency(statisticsTotal)}
                  </p>
                </div>
                ...
                {paymentMethodStatistics.map((item) => (
                  ...
                  >
                    ...
                    >
                      <div
                        className={cn(
                          "chart-bar h-full rounded-full",
                          item.tone.bar
                        )}
                        style={{
                          width: getChartWidth(item.total, statisticsTotal)
                        }}
                      />
                    </div>
                    <p
                    ...
                    <>
                      <div className="mt-5 flex h-4 overflow-hidden rounded-full bg-slate-100">
                        {statusStatistics.map((item) => {
                          const tone =
                            statusChartTone[item.status] ??
                            chartTones[chartTones.length - 1];
                          return (
                            <div
                              key={item.status}
                              className={cn("chart-bar", tone.bar)}
                              style={{
                                width: `${(item.total / statisticsTotal) * 100}%`
                              }}
                  ...
                  <div className="mt-5 space-y-4">
                    {billRanking.map((item, index) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_9rem] sm:items-center"
                        >
                          <div className="min-w-0">
                            <div className="flex items-center justify-between gap-3 text-sm">
                              <span className="min-w-0 truncate font-medium text-slate-800">
                            ...
                            <div
                              className={cn(
                                "mt-2 h-3 overflow-hidden rounded-full",
                                tone.track
                              )}
                            >
                              <div
          ...
          <div className="soft-dialog panel max-h-[calc(100vh-1rem)] w-full max-w-lg overflow-y-auto rounded-b-none sm:rounded-lg">
            ...
            <div className="space-y-3 p-5">
              ...
              {groups.map((groupName, index) => {
                ...
                const itemCount = localItems.filter(
                  (item) => item.group === groupName
                ).length;
                const visibleCount = groups.filter(
                  (item) => !hiddenGroups.includes(item)
                ).length;
                return (
                  <div
                    key={groupName}
                    ...
                    <span className="flex shrink-0 items-center justify-between gap-2 sm:justify-end">
                      ...
                      <span className="flex items-center gap-1">
                        <button
                          ...
                          type="button"
                          disabled={
                            index === 0 || savingGroupOrder || creatingGroup
                          }
...
function moveGroup(
  ...
  [nextGroups[currentIndex], nextGroups[nextIndex]] = [
    nextGroups[nextIndex],
