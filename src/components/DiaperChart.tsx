...
export function DiaperChart({ items }: { items: Item[] }) {
  ...
  const activeDay = selectedDay && buckets.some((bucket) => bucket.key === selectedDay) ? selectedDay : dateKey(new Date());
  const dayRecords = range === "周" ? visible.filter((item) => dateKey(new Date(item.happenedAt)) === activeDay).sort((a, b) => new Date(a.happenedAt).getTime() - new Date(b.happenedAt).getTime()) : [];
  ...
  return <section className="mb-4 rounded-[2rem] border border-white/80 bg-white/60 p-4 shadow-xl shadow-indigo-200/25 backdrop-blur-2xl md:p-5">
    <div className="flex items-start justify-between gap-3">
      ...
      <div className="flex rounded-full bg-white/70 p-1 ring-1 ring-white/80">{ranges.map((value) => <button key={value} type="button" onClick={() => { setRange(value); setSelectedDay(null); }} className={`rounded-full px-3 py-1.5 text-xs transition ${range === value ? "bg-indigo-300/80 text-slate-700 shadow-sm" : "text-slate-500"}`}>{value}</button>)}</div>
    </div>
...
function filterItems(items: Item[], range: Range) { const now = Date.now(); const today = dateKey(new Date(now)); const days = range === "周" ? 7 : range === "月" ? 30 : 365; const firstDay = dateKey(new Date(now - (days - 1) * 86400000)); return items.filter((item) => { const time = new Date(item.happenedAt).getTime(); if (!Number.isFinite(time) || time > now + 60000) return false; const day = dateKey(new Date(time)); return range === "日" ? day === today : day >= firstDay && day <= today; }); }
...
function clock(value: string) { return new Date(value).toLocaleTimeString("zh-CN", { timeZone: zone, hour: "2-digit", minute: "2-digit", hour12: false }); }
